...
test_that("clean_open_library_result works", {
  ...
  result_without_nested_JSON <- result |> 
  dplyr::select(-tidyselect::contains("JSON"))
...
purrr::map(json, ~.x$details)  |> 
 ...
 purrr::map(json, ~.x$details)  |> 
  purrr::map(~.x$publishers)



  purrr::map(~.x$name) 
  
purrr::map(json, ~.x$authors)

purrr::map(json, tidyr::unnest)

  
   |> 
  tibble::as_tibble()  |> 
  tidyr::unnest(cols = c(`ISBN:2365772013`))
})
```
